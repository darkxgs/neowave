...
export function middleware(request: NextRequest) {
  ...
  // Try both possible cookie names and auth_status fallback
  ...
  if (isProtectedRoute && !authCookie && authStatus?.value !== "authenticated") {
  ...
  if ((path === "/login" || path === "/admin-login" || path === "/user-login") && (authCookie || authStatus?.value === "authenticated")) {
