...
export async function login(username: string, password: string) {
  ...
  try {
    if (username === "admin" && password === "neowave342") {
      ...
      // Try to store session in PostgreSQL, but don't fail if it doesn't work
      let dbSuccess = false
      try {
        await sessionDb.create(token, { username })
        dbSuccess = true
        console.log("PostgreSQL session stored successfully")
      } catch (dbError) {
